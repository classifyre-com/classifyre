 ...
 */
function collectKeyOrder(obj: JsonObject, prefix = ""): Array<[string, string[]]> {
...
function runParityChecks(
  ...
  for (const [path, baseKeys] of baselineOrder) {
    ...
    if (!targetEntry) {
      throw new Error(`[${targetName}] Object at path "${path}" is missing in target`)
    }
...
describe("i18n key parity", () => {
  it("de.json has identical keys and structure to en.json", () => {
    runParityChecks(en as unknown as JsonObject, de as unknown as JsonObject, "de.json")
  })
})
