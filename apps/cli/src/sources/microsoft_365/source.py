...
class Microsoft365Source(BaseSource):
    ...
    def _build_credential(self) -> Any:
        ...
        if isinstance(required, Microsoft365RequiredClientSecret):
            ...
            password_bytes = (
                masked.certificate_password.encode("utf-8")
                if masked.certificate_password
                else None
            )
    ...
    def _iter_drive_items(
        ...
        if path_prefix:
            ...
            root_url = (
                f"https://graph.microsoft.com/v1.0/drives/{drive_ref.drive_id}/root/children"
            )
    ...
    def _recurse_folder(
        ...
        page_items = self._graph_paged_list(
            url,
            params={"$select": "id,name,size,lastModifiedDateTime,file,folder,webUrl,eTag,createdBy,lastModifiedBy,parentReference"},
        )
    ...
    def _matches_extension_filters(self, filename: str) -> bool:
        ...
        if include:
            normalized_include = [e.lower() if e.startswith(".") else f".{e.lower()}" for e in include]
            if ext not in normalized_include:
                return False

        exclude = scope.exclude_extensions
        if exclude:
            normalized_exclude = [e.lower() if e.startswith(".") else f".{e.lower()}" for e in exclude]
    ...
    def _apply_sampling(self, items: list[DriveItemRef]) -> list[DriveItemRef]:
    ...
    async def extract_raw(self) -> AsyncGenerator[list[SingleAssetScanResults], None]:
            ...
            for site in sites:
                ...
                for drive in drives:
                    if include_drives:
                        ...
                        all_drives.append((drive, Microsoft365Ecosystem.sharepoint_sites.value, drive_asset.hash))
                        batch.append(drive_asset)
                    else:
                        drive_raw_id = f"m365_drive_#_{drive.drive_id}"
                        drive_hash = self.generate_hash_id(drive_raw_id)
                        all_drives.append((drive, Microsoft365Ecosystem.sharepoint_sites.value, drive_hash))
        ...
        if Microsoft365Ecosystem.onedrive.value in ecosystems:
            ...
            for drive in drives:
                ...
                if include_drives:
                    ...
                    all_drives.append((drive, Microsoft365Ecosystem.onedrive.value, drive_asset.hash))
                    batch.append(drive_asset)
                else:
                    drive_raw_id = f"m365_drive_#_{drive.drive_id}"
                    drive_hash = self.generate_hash_id(drive_raw_id)
        ...
        if Microsoft365Ecosystem.teams_files.value in ecosystems:
            ...
            for drive in drives:
                ...
                if include_drives:
                    drive_asset = self._drive_to_asset(drive, None)
                    all_drives.append((drive, Microsoft365Ecosystem.teams_files.value, drive_asset.hash))
                    batch.append(drive_asset)
                else:
                    drive_raw_id = f"m365_drive_#_{drive.drive_id}"
                    drive_hash = self.generate_hash_id(drive_raw_id)
        ...
        for drive_ref, ecosystem, drive_hash in all_drives:
            ...
            sampled = self._apply_sampling(items)

            logger.info(
                "Processing %d/%d items from drive '%s' (%s)",
                len(sampled), len(items), drive_ref.display_name, ecosystem,
            )
