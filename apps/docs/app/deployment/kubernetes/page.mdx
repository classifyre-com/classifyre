...
Both require `ReadWriteMany` access mode. On clusters without a `ReadWriteMany` storage class (e.g. bare k3s), set a compatible `storageClassName` or point them at an existing NFS/CSI-backed PVC:
