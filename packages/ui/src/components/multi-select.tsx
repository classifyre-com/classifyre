  ...
  if (single && selectedValues.size > 0) {
    ...
    return (
      <span className="min-w-0 overflow-hidden">
        {selectedValue ? items.get(selectedValue) : null}
      </span>
    );
  }
      ...
      {[...selectedValues]
        ...
        .map((value) => (
          <Badge
            variant="outline"
            data-selected-item
            className="group flex items-center gap-1 border-accent bg-accent text-accent-foreground"
            key={value}
            onClick={
              clickToRemove
                ? (e) => {
          ...
          >
            {items.get(value)}
            {clickToRemove && (
              <XIcon className="size-2 text-accent-foreground/70 group-hover:text-accent-foreground" />
            )}
          </Badge>
        ))}
      <Badge
