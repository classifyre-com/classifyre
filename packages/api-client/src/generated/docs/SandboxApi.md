...
async function example() {
  ...
  const body = {
    ...
    contentType: ...,
    // Array<'SECRETS' | 'PII' | 'TOXIC' | 'NSFW' | 'YARA' | 'BROKEN_LINKS' | 'PROMPT_INJECTION' | 'SPAM' | 'LANGUAGE' | 'CODE_SECURITY' | 'CUSTOM'> | Filter by detectors used in the run configuration or reported findings (optional)
    detectorType: ...,
...
| **detectorType** | `SECRETS`, `PII`, `TOXIC`, `NSFW`, `YARA`, `BROKEN_LINKS`, `PROMPT_INJECTION`, `SPAM`, `LANGUAGE`, `CODE_SECURITY`, `CUSTOM` | Filter by detectors used in the run configuration or reported findings | [Optional] [Enum: SECRETS, PII, TOXIC, NSFW, YARA, BROKEN_LINKS, PROMPT_INJECTION, SPAM, LANGUAGE, CODE_SECURITY, CUSTOM] |
